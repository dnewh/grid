    ...
    match action {
        Action::SchemaCreate(_) => schemas_yaml
            ...
            .map(|schema_yaml| {
                ...
                let schema_owner = parse_value_as_string(schema_yaml, "owner")?.ok_or_else(|| {
...
- name: "Phone"
  ...
  properties:
    ...
    fn make_update_schema_payload_1() -> SchemaPayload {
        generate_update_schema_payload("Lightbulb", "013600", &create_lightbulb_property_definitions())
            .unwrap()
    }

    fn make_update_schema_payload_2() -> SchemaPayload {
        generate_update_schema_payload("Phone", "013600", &create_phone_property_definitions()).unwrap()
    }

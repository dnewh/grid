...
pub(in crate::grid_db::locations) trait LocationStoreListLocationsOperation<C: Connection> {
    fn list_locations(
        &self,
        ...
        self.conn
            ...
            .run::<_, LocationStoreError, _>(|| {
                ...
                for l in locs {
                    ...
                    // let attr = location_attribute::table
                    //     .select(location_attribute::all_columns)
                    //     .filter(location_attribute::location_id
                    //         .eq(&loc.location_id)
                    //         .and(location_attribute::service_id.eq(&loc.service_id))
                    //         .and(location_attribute::end_commit_num.eq(MAX_COMMIT_NUM)))
                    //     .load::<LocationAttributeModel>(self.conn)
                    //     .map(Some)
                    //     .map_err(|err| LocationStoreError::OperationError {
                    //         context: "Failed to fetch locations".to_string(),
                    //         source: Some(Box::new(err)),
    ...
    fn get_attributes(
        ...
        for attr in attributes {
            ...
            if children.is_empty() {
                attrs.push(LocationAttribute::from_model(attr));
            } else {
                attrs.push(LocationAttribute::from_model_with_children(
                    attr,
